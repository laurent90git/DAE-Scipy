...
"""
  ...
  2) Numerical solution
    ...
    error on the algebraic variables is forced to one, when these are clearly 
    identified (those that correspond to a row of zeros in M).
...
rtol=1e-6; atol=1e-6 # relative and absolute tolerances for time adaptation
...
lbda_0 = m*r0* m*g*np.cos(theta_0)/r0 # equilibrium along the rod's axis
...
if chosen_index==3:
  ...
  def jac_dae(t,X):
    ...
    return np.array([[0.,0.,1.,0.,0.],
                     [0.,0.,0.,1.,0.],
                     [-2*lbda/m, 0., 0., 0., -2*x/m],
                     [0.,-2*lbda/m,  0., 0., -2*y/m],
                     [-2*x*lbda, -2*y*lbda -m*g/2, m*vx, m*vy, (x**2+y**2)]])
                     # [-8*x/m*lbda, -8*y/m*lbda, 4*vx, 4*vy-2*g, 0.]])
...
jac_dae = lambda t,x: scipy.optimize._numdiff.approx_derivative(fun=lambda x: dae_fun(t,x),
                                                                x0=x, method='cs',
...
method=RadauDAE
bPrint=True
# else: # ODE, we can a high-order explicit algorithm
#   rtol=atol=1e-12
#   method='DOP853'
Xini = np.array([x0,y0,0.,0.,lbda_0]) # the initial condition should be
...
if method==RadauDAE:
  ...
  print("ODE {} in {} time steps, {} fev".format(
...
fig, ax = plt.subplots(5,1,sharex=True, dpi=300, figsize=np.array([1.5,3])*5)
i=0
...
i+=1
ax[i].semilogy(sol.t[:-1], np.diff(sol.t), color='tab:blue', linestyle='-', marker='.', linewidth=1, label=None)
ax[i].grid()
ax[i].set_ylabel(r'$\Delta t$ (s)')

# condition numbers for the LU solves in the Newton loop
i+=1
ax[i].semilogy(sol.solver.info['cond']['t'], sol.solver.info['cond']['LU_real'], color='tab:blue', linestyle='-', linewidth=2, label='cond(real)')
ax[i].semilogy(sol.solver.info['cond']['t'], sol.solver.info['cond']['LU_complex'], color='tab:orange', linestyle='-', linewidth=1, label='cond(complexe)')
ax[i].legend(frameon=False)
ax[i].grid()
