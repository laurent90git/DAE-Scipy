...
eigvals  = np.array([float(i) for i in range(1,n+1)]) # eigenvalues of the reference ODE
...
sol_with_mass = solve_ivp(fun=modelfun_with_mass, t_span=(0., tf), y0=y0, max_step=np.inf,
                rtol=1e-10, atol=1e-10, jac=None, jac_sparsity=None, method=method,
...
assert_allclose(sol_without_mass.y[:,-1], true_solution, rtol=1e-10, err_msg='the solution without mass-matrix is not coherent with the theoretical solution')
assert_allclose(sol_with_mass.y[:,-1],    true_solution, rtol=1e-10, err_msg='the solution with the mass-matrix is not coherent with the theoretical solution')
assert_(sol_without_mass.t.size==sol_with_mass.t.size, msg='the mass-matrix option should not affect the number of steps')
