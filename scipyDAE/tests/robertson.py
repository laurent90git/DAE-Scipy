...
def generateSystem(bDAE=False):
  
  if not bDAE: # Define the ODE formulation
    mass     = np.eye(3)
    # mass     = diags([1.,1.,0.], 0, format='csc') #sparse format
    fun = lambda t,y: np.array([-0.04*y[0] + 1e4*y[1]*y[2],
                                0.04*y[0] - 1e4*y[1]*y[2] - 3e7*(y[1]**2),
                                3e7*(y[1]**2)])
    var_index = np.array([0,0,0]) # algebraic index of the components
    jac = lambda t,y: np.array([[-0.04,  1e4*y[2],               1e4*y[1]],
                                [0.04,  -1e4*y[2] - 2*3e7*y[1], -1e4*y[1]],
                                [0.,     2*3e7*y[1],                        0.],])
  else: # Define the DAE system
  ...
  return [0., tf], y0, fun, jac, mass, var_index

if __name__ == "__main__":
  
  t_span, y0, modelfun_DAE, jac_dae, mass_matrix, var_index = generateSystem(True)
  _, _, modelfun_ODE, jac_ode, _, _= generateSystem(False)
  
  # jac_ode=None
  rtol=1e-3; atol=rtol/10 #1e-6 # absolute and relative tolerances for time step adaptation
  
  
  # solve the DAE formulation
  t1 = pytime.time()
  sol_dae = solve_ivp(fun=modelfun_DAE, t_span=t_span, y0=y0, max_step=np.inf,
                      ...
                      scale_error = True,
                      mass_matrix=mass_matrix, bPrint=bPrint,
                      bReport=True)
  t2 = pytime.time()
  ...
  t1 = pytime.time()
  sol_ode = solve_ivp(fun=modelfun_ODE, t_span=t_span, y0=y0, max_step=np.inf,
                      rtol=rtol, atol=atol, jac=jac_ode, jac_sparsity=None,
                      method=method, vectorized=False, first_step=1e-8, dense_output=True,
                      max_newton_ite=6, max_bad_ite=0,
                      scale_residuals = True,
                      scale_newton_norm = False,
  ...
  test_t = np.logspace(-3,np.log10(t_span[1]),10000) # avoid initial transient with values close to zero
