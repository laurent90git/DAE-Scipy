...
def generateSystem():
  ...
  tf = 1. # final time
  var_index = None
  return [0., tf], y0, modelfun_DAE, jac_dae, mass, var_index, Ue

if __name__ == "__main__":
  t_span, y0, modelfun_DAE, jac_dae, mass, var_index, Ue = generateSystem()
  
  # 1 - solve the ODE
  rtol=1e-5; atol=1e-5
  
  sol = solve_ivp(fun=modelfun_DAE, t_span=t_span, y0=y0, max_step=np.inf,
